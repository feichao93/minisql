...
public class IndexManager {
    ...
    public static void main(String[] args) {
        BTree<Integer> testTree = new BTree<>();
...
class BTree<T> implements Serializable {
    ...
    private final static int ORDER = IndexManager.ORDER;
    private final static int LEFT = IndexManager.LEFT;
    private final static int RIGHT = IndexManager.RIGHT;
    private Node<T> root;
    ...
    private Node<T> insert(Node<T> node, T element, int shift) {
        int index;
        if (node instanceof LeafNode)  // leaf node
        {
            ...
            {
                LeafNode<T> returnLeafNode = new LeafNode<>(ORDER);
        ...
        } else // non-leaf node
        {
            ...
            if (newNode != null) // there is new node created  // newNode should be in children[index+1]
            {
                if (node.childNum == ORDER) // need to split
                {
                    Node<T> returnNode = new Node<>(ORDER);   // returnNode will be the right node
    ...
    private int search(Node<T> node, T element) {
        if (node instanceof LeafNode) {
            int index = searchDeleteIndex(node.keys, node.childNum, element);
            if (index == -1)
                return -1;
            else
                return ((LeafNode<T>) node).shifts.get(index);
        } else {
    ...
    private void mergeChild(Node<T> left, Node<T> right) {
    ...
    private static <K> int searchDeleteIndex(ArrayList<K> keys, int length, K element) {
    ...
    private static <K> int searchInsertIndex(ArrayList<K> keys, int length, K element) {
...
//			return 0;
    ...
    private static <K> int compare(K left, K right) {
    ...
    private static <K> void copyArrayList(ArrayList<K> source, int startIndex, int endIndex, ArrayList<K> destination) {
        for (int i = startIndex; i < endIndex; i++) {
            destination.set(i - startIndex, source.get(i));
        }
    }

    private static <K> void shiftArrayList(ArrayList<K> list, int startIndex, int endIndex, int shift)
    ...
    private List<Integer> getAllShifts(Node<T> node) {
        if (node instanceof LeafNode) {
